...
let
  ...
  pkgsDocs = pkgs.extend (final: prev: {
  ...
  nixosEval = pkgsDocs.lib.evalModules {
    ...
    specialArgs = { pkgs = pkgsDocs; };
  };
  nixosDocs = pkgsDocs.nixosOptionsDoc {
    options = builtins.removeAttrs nixosEval.options [ "_module" "systemd" ];
  };
  ...
  hmDocs = pkgsDocs.nixosOptionsDoc {
    options = builtins.removeAttrs hmEval.options [ "_module" "systemd" ];
  };
in
