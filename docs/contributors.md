...
`crud` is the common case. A config object with no key is `sync = singleton`; and an API that doesn't fit either (multi-endpoint writes, server-generated GUID ids, whole-list replaces — e.g. Jellyfin users/libraries/repositories) is `sync = custom` with a hand-written reconcile hook. Non-*arr apps that serialise PascalCase add `case = pascal`. The `core-architecture` skill covers these; reach for them only when the plain `crud`/`singleton` shape doesn't fit.
