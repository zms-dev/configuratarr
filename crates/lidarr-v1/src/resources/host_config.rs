...
pub struct HostConfig {
    #[id]
    pub id: Option<i32>,
    /// IP address or hostname Lidarr binds to; `*` binds to all interfaces.
    pub bind_address: Option<String>,
    /// HTTP port Lidarr listens on.
    pub port: i32,
    /// HTTPS port Lidarr listens on when SSL is enabled.
    pub ssl_port: i32,
    /// Enables HTTPS/TLS for the Lidarr web UI.
    pub enable_ssl: bool,
    /// Opens the Lidarr web UI in the default browser on startup.
    pub launch_browser: bool,
    /// Authentication method for the Lidarr web UI: `none`, `basic`, `forms`, or `external`.
    #[default("none")]
    pub authentication_method: String,
    /// Whether authentication is required: `enabled` or `disabledForLocalAddresses`.
    #[default("enabled")]
    pub authentication_required: String,
    /// Sends anonymised usage and error data to the Lidarr team.
    ...
    /// Update channel or branch Lidarr checks for updates (e.g. `main`, `develop`).
    pub branch: Option<String>,
    /// Lidarr API key used to authenticate API requests.
    ...
    /// URL base path for reverse-proxy deployments (e.g. `/lidarr`).
    pub url_base: Option<String>,
    /// Display name for this Lidarr instance shown in the browser title and notifications.
    pub instance_name: Option<String>,
    /// Externally reachable URL for this instance, used in notifications.
    pub application_url: Option<String>,
    /// Allows Lidarr to update itself automatically when a new version is available.
    pub update_automatically: bool,
    /// How Lidarr applies updates: `builtIn`, `script`, `external`, `apt`, or `docker`.
    #[default("docker")]
    ...
    /// Routes Lidarr's outbound HTTP traffic through a proxy server.
    ...
    /// Folder path where Lidarr stores automatic database backups.
