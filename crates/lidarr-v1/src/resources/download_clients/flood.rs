...
pub struct FloodConfig {
    ...
    /// Tags applied to the torrent in Flood (string labels, not Lidarr tag ids).
    /// Note: wire name is `tags`; the Lidarr API uses a field exception mapping.
    #[wire(name = "tags")]
    pub field_tags: Vec<String>,
    /// Additional Lidarr-managed metadata tags appended to the torrent (integer codes).
    #[wire(name = "additionalTags")]
    pub additional_tags: Vec<i32>,
    /// Tags applied to the torrent in Flood after Lidarr imports it.
