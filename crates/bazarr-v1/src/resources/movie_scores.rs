...
/// Movie subtitle-match scoring weights (`settings-movie_scores-*`).
///
/// Each field is the score a match on that attribute contributes; the total is
/// compared against `general.minimum_score_movie` to decide whether a subtitle
/// is good enough to download.
#[nested(case = snake)]
pub struct MovieScores {
    /// Points for a release-hash match (exact file match).
    pub hash: Option<i32>,
    /// Points for a matching title.
