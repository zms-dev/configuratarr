...
//! provider-style resource (download clients, indexers, import lists,
//! notifications): identity, tag refs, and the read-only metadata the API
//! returns. Deduped here so each concrete provider struct shows only its
