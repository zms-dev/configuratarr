...
//! Radarr OpenAPI request schemas.
//!
...
//! The radarr schemas are `additionalProperties: false`, so a hallucinated,
...
use radarr_v3::resources;
use serde_json::{Value, json};

fn spec() -> Value {
    serde_json::from_str(include_str!("../spec/radarr-v3.json")).expect("spec parses")
}
...
// ── providers (envelope only; inner fields-blob not in static spec) ───────────
conformance!(
    download_client,
...
conformance!(
    metadata_config,
    resources::metadata_config::MetadataConfig,
    "MetadataConfigResource",
    "testdata/metadata_config/config.yaml"
);
conformance!(
    ui_config,
